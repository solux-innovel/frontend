...
const fetchRecommendedPlot = async (concept: string, topic: string, background: string, character: string) => {
...
const Create_7 = ({ route }) => {
  ...
  const handlePressButton1 = async () => {
    ...
    setTimeout(async () => {
      ...
      try {
        const newRecommendedPlot = await fetchRecommendedPlot(savedConcept, savedTopic, savedBackground, savedCharacter);
